...
bool E88G4_RPC::IsInside(const G4ThreeVector &gPos) const {
   G4ThreeVector lPos = GetLocalPosition(gPos);
   if(vSolid_->Inside(lPos) != kOutside) return true;
   else return false;
}

G4VPhysicalVolume* E88G4_RPC::Place(G4LogicalVolume *pMother){
...
//new sako
G4VPhysicalVolume* E88G4_RPC::PlaceRPC(G4LogicalVolume *pMother){//strip layer position
  ...
  G4cout << "E88G4_RPC::PlaceRPC : ID = " << id_ << ", pos = " << pos_RPC/mm << G4endl;
...
/*
G4VPhysicalVolume* E88G4_RPC::PlaceSensor(G4LogicalVolume *pMother){//strip layer position
  ...
  //  }
   ...
   G4cout << "E88G4_RPC::PlaceSensor : ID = " << id_ << ", pos = " << pos_sensor/mm << G4endl;
