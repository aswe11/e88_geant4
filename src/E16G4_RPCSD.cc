...
E16G4_RPCSD::E16G4_RPCSD(const G4String &name, const E16ANA_GeometryV2 *geom) : 
...
E16G4_RPCSD::~E16G4_RPCSD(){
}

void E16G4_RPCSD::SetElement(int id, E16G4_BeamLineElement *elem){
   elem_[id] = elem;
}

void E16G4_RPCSD::Initialize(G4HCofThisEvent *HCTE){
   for(int i=0; i<n_rpc; i++){
...
G4bool E16G4_RPCSD::ProcessHits(G4Step *astep, G4TouchableHistory *ROhist){
   ...
   if(id >= 0){
      ...
      if(geom_->RPC(id)->IsCrossed(gpos_pre, gpos_post, lpos_cross)){
         ...
         E16G4_RPCHit *hit = new E16G4_RPCHit(lpos, time, charge);
         hit->SetGPos(gpos);
         hit->SetLPos(lpos);
         hit->SetGMom(gmom);
         hit->SetLMom(lmom);
...
void E16G4_RPCSD::EndOfEvent(G4HCofThisEvent *){
}

void E16G4_RPCSD::DrawAll(){
}

void E16G4_RPCSD::PrintAll(){
   for(int i=0; i<n_rpc; i++){
