...
//E88_G4OutputData.cc 150120 by S. Yokkaichi
...
//------------------------------------------------------
int E88_G4Event::WriteData(FILE* fpWrite){
...
int E88_G4Event::ReadData(FILE* fpRead){

    int flag=eventHeader.ReadData(fpRead);
    //cerr<<"E88_G4OutputData L38 "<<flag<<endl;
    if (flag != 1 ) {
      E16INFO("eof");
      return 0;
    }
...
//------------------------------------------------------
E88_G4OutputData::E88_G4OutputData(){
    fpRead=NULL;
    fpWrite=NULL;
    overlapEvent = new E88_G4Event();
}
...
int E88_G4OutputData::CloseReadFile(){
  fclose(fpRead); fpRead=NULL;
  return 0;
}
int E88_G4OutputData::CloseWriteFile(){
  fclose(fpWrite); fpWrite=NULL;
  return 0;
}

int E88_G4OutputData::OpenReadFile(const char* filename){
  if( fpRead != NULL ){
...
int E88_G4OutputData::OpenWriteFile(const char* filename){
  if( fpWrite != NULL ){
...
int E88_G4Event::EventAppend(double timeoffset,
				E88_G4Event& ev2, int eventIDoffset  ){
...
int E88_G4OutputData::ReadOverlapEvent(int multi, double* timeInterval, 
int jamInteractionFlag){
  delete overlapEvent;
  overlapEvent = new E88_G4Event();//in order to clear the previous.
...
//--------------------------------------
void E88_G4ModuleHitList::HitPrint(){
...
void E88_G4ModuleHitList::OverlapGTRSingleHits(){

  for (int k=0; k< NmoduleGTR ;k++){   //module-by-module
    vector<E88_G4Hit>& vec1 = GTRModule(0, k);
    vector<E88_G4Hit>& vec2 = GTRModule(1, k);
    vector<E88_G4Hit>& vec3 = GTRModule(2, k);
...
void E88_G4ModuleHitList::OverlapSSDSingleHits(){
  ...
  for(int k=0; k<NmoduleSSD; k++){
     vector<E88_G4Hit> &vec = SSDModule(k);
     E16ANA_EventOverlap::InsertSingleRateBKG_SSD(k, vec, is_forward[k], halo_angle[k]);
  }
}

void E88_G4ModuleHitList::OverlapGTRSingleHits151201(){
...
void E88_G4ModuleHitList::OverlapGTRSingleHits(){
  ...
  for (int k=0; k< NmoduleGTR ;k++){   //module-by-module
    ...
    vector<E88_G4Hit> vec1s;  E16ANA_EventOverlap::SingleRate100(k, vec1s);
    vector<E88_G4Hit> vec2s;  E16ANA_EventOverlap::SingleRate200(k, vec2s);
    vector<E88_G4Hit> vec3s;  E16ANA_EventOverlap::SingleRate300(k, vec3s);
...
void E88_G4ModuleHitList::SetG4Event(E88_G4Event& event){

  for(int layerID=0; layerID<3; layerID++){
    int jmax = event.GTRs( layerID ).Noh();
    //    cerr<<"jmax "<<jmax<<endl;
    for (int j=0; j< jmax ;j++){
      E88_G4Hit & hit = event.GTRs( layerID ).Hit(j);//reference
  ...
  //    cerr<<"jmax "<<jmax<<endl;
  for (int j=0; j< jmax ;j++){
    E88_G4Hit & hit = event.HBD().Hit(j);//reference
    int detID = hit.DetectorID();
    int pid = hit.PID();
    //      cerr<<"detID "<<detID<<endl;
    if( pid != 22 ){ // i.e. not gamma
  ...
  //    cerr<<"jmax "<<jmax<<endl;
  for (int j=0; j< jmax ;j++){
    E88_G4Hit & hit = event.RPC().Hit(j);//reference
    int detID = hit.DetectorID();
    int pid = hit.PID();
    //      cerr<<"detID "<<detID<<endl;
    if( pid != 22 ){ // i.e. not gamma
  ...
  //    cerr<<"jmax "<<jmax<<endl;
  for (int j=0; j< jmax ;j++){
    E88_G4Hit & hit = event.LGVD().Hit(j);//reference
    int detID = hit.DetectorID();
    int pid = hit.PID();
    //      cerr<<"detID "<<detID<<endl;
    if( pid != 22 ){ // i.e. not gamma
...
void E16ANA_EventOverlap::InsertSingleRateBKG(int flag, int layerID, 
  int moduleID, double mean, double size, std::vector<E88_G4Hit>& hits){
...
void E16ANA_EventOverlap::InsertSingleRateBKG_SSD(int moduleID, std::vector<E88_G4Hit> &hits, int is_forward, double halo_angle){
   double size = 50.0; // [mm]
...
void E16ANA_EventOverlap::InsertSingleRateBKG_SSD(int flag,
  int moduleID, double rate, double size, std::vector<E88_G4Hit>& hits, const TVector3 &halo_direction){
...
#if 0
void SingleRate100(int detID, vector<E88_G4Hit>& hits){
...
void E16ANA_EventOverlap::InsertSingleRateBKG100(
  int moduleID, std::vector<E88_G4Hit>& hits){
...
void E16ANA_EventOverlap::InsertSingleRateBKG200(int moduleID, std::vector<E88_G4Hit>& hits){
...
void E16ANA_EventOverlap::InsertSingleRateBKG300(int moduleID, std::vector<E88_G4Hit>& hits){
...
void E16ANA_EventOverlap::InsertSingleRateBKG151201(double moduleID, double mean, double size, std::vector<E88_G4Hit>& hits){
  double half = 0.5*size;
  double timeInterval[1024]={0};
...
void E16ANA_EventOverlap::InsertSingleRateBKG200_151201(int moduleID, std::vector<E88_G4Hit>& hits){
  //    double mean =  12.5; // /525nsec for 25MHz = 1.25kHz/mm2*200*200/2
  double mean =  mean200;
  double size =20;//cm
  InsertSingleRateBKG151201(moduleID, mean, size, hits);
...
void E16ANA_EventOverlap::InsertSingleRateBKG300_151201(int moduleID, std::vector<E88_G4Hit>& hits){
  //    double mean =  12.5; // /525nsec for 25MHz = 5kHz/mm2*300*300
  double mean =  mean300;
  double size =30;//cm 
  InsertSingleRateBKG151201(moduleID, mean, size, hits);
