...
std::ostream& operator<<(std::ostream &os, const E88_GeometryV2::Params_t &rhs){
   for(int i=0; i<3; i++){
...
std::istream& operator>>(std::istream &is, E88_GeometryV2::Params_t &rhs){
   double temp;
   for(int i=0; i<3; i++){
      is >> temp;
      rhs.xyz[i] = temp;
...
E88_GeometryV2::Node_t::Node_t(const std::string &_name) : 
   name(_name)
{
}

E88_GeometryV2::Node_t::~Node_t(){
   //std::cout << "E88_GeometryV2::Node_t delete, name = " << name << std::endl;
...
//void E88_GeometryV2::Node_t::AddChild(const std::string &key, Node_t *node){
void E88_GeometryV2::Node_t::AddChild(Node_t *node){
   std::string key = node->GetName();
   children[key] = node;
}

void E88_GeometryV2::Node_t::Translate(const TVector3 &delta_r){
   std::map<std::string, Node_t*>::iterator it;
   for(it = children.begin(); it != children.end(); it++){
      Node_t *node = it->second;
...
void E88_GeometryV2::Node_t::Rotate(double alpha, double beta, double gamma, const TVector3 &rotation_center){
   std::map<std::string, Node_t*>::iterator it;
   for(it = children.begin(); it != children.end(); it++){
      Node_t *node = it->second;
...
void E88_GeometryV2::Node_t::Rotate(double alpha, double beta, double gamma){
   std::map<std::string, Node_t*>::iterator it;
   for(it = children.begin(); it != children.end(); it++){
      Node_t *node = it->second;
...
void E88_GeometryV2::Node_t::Reset(){
   std::map<std::string, Node_t*>::iterator it;
   for(it = children.begin(); it != children.end(); it++){
      Node_t *node = it->second;
      node->Reset();
...
E88_GeometryV2::Node_t* E88_GeometryV2::Node_t::FindChild(const std::string &key){
...
void E88_GeometryV2::Node_t::TranslateAndRotate(const Params_t &_par){
   this->Rotate(_par.rot, _par.rot_center);
   this->Translate(_par.xyz);
}

void E88_GeometryV2::Node_t::TranslateAndRotate(const std::string &tree_str, ParamsArray_t &_params){
   std::map<std::string, Node_t*>::iterator it;
   for(it = children.begin(); it != children.end(); it++){
      Node_t *node = it->second;
...
void E88_GeometryV2::Node_t::Print(int depth){
   std::cout << "E88_GeometryV2::Node_t::Print() : " << std::setw((depth)*4) << "" << name <<std::endl;
   std::map<std::string, Node_t*>::iterator it;
   for(it = children.begin(); it != children.end(); it++){
      Node_t *node = it->second;
      node->Print(depth+1);
...
E88_GeometryV2::Leaf_t::Leaf_t(const std::string &_name, E16ANA_DetectorGeometry *_detector) : 
   Node_t(_name), detector(_detector)
{
}

E88_GeometryV2::Leaf_t::~Leaf_t(){
   //std::cout << "E88_GeometryV2::Leaf_t delete" << std::endl;
   if(detector != NULL){
      delete detector;
   }
}

const std::string E88_GeometryV2::delimiter   = ":";
const std::string E88_GeometryV2::design_str  = "D"; // Design
const std::string E88_GeometryV2::error_str   = "A"; // Adjust
const std::string E88_GeometryV2::e16_str     = "E16";
const std::string E88_GeometryV2::gtr_str     = "GTR";
const std::string E88_GeometryV2::hbd_str     = "HBD";
const std::string E88_GeometryV2::ssd_str     = "SSD";
//sako
const std::string E88_GeometryV2::rpc_str     = "RPC";
const std::string E88_GeometryV2::lgvd_str    = "LGVD";
const std::string E88_GeometryV2::frame_str   = "Frame";
const std::string E88_GeometryV2::layer_str   = "Layer";
const std::string E88_GeometryV2::chamber_str = "Chamber";

E88_GeometryV2::E88_GeometryV2() : 
  //org   n_gtr_modules(33), n_gtr_layers(3), n_ssd_modules(33), n_hbd_modules(27), n_lg_modules(27)
  //sako
  n_gtr_modules(33), n_gtr_layers(3), n_ssd_modules(33), n_rpc_modules(81), n_hbd_modules(27), n_lg_modules(27)
{
   Initialize();
   SetDesignValues();
   std::cerr << "E88_GeometryV2 initialized by design values." << std::endl;
}

E88_GeometryV2::E88_GeometryV2(const std::string &filename) :
  //org   n_gtr_modules(33), n_gtr_layers(3), n_ssd_modules(33), n_hbd_modules(27), n_lg_modules(27)
  //sako
  n_gtr_modules(33), n_gtr_layers(3), n_ssd_modules(33), n_rpc_modules(81), n_hbd_modules(27), n_lg_modules(27)
{
  //org  std::ifstream ifs(filename);
  std::ifstream ifs(filename.c_str());
   if(ifs.fail()){
      std::cerr << __func__ << " (" << __FILE__ << "/" << __LINE__ << ") exit: geom file cannot open...: " << filename << std::endl;
      exit(1);
   }
   std::cerr << "E88_GeometryV2 initialized by " << filename << std::endl;
...
void E88_GeometryV2::Initialize(){
...
void E88_GeometryV2::ConstructGeometryTree(){
   ...
   //sako
   rpc_geometry = new E16ANA_DetectorGeometry*[n_rpc_modules];
   ...
   //sako
   for(int i=0; i<n_rpc_modules; i++){
      node_name = GetNumString(chamber_str, i);
      rpc_geometry[i] = new E16ANA_PlanarGeometry(rpc_str,i,0);
      rpc_leaf[i] = new Leaf_t(node_name, rpc_geometry[i]);
   }
   ...
   // construct RPC tree, sako
   for(int i=0; i<n_rpc_modules/9; i++){
      node_name = GetNumString(frame_str, i);
      Node_t *rpc_frame = new Node_t(node_name);
      rpc_frame->AddChild(rpc_leaf[i*9+0]);
      rpc_frame->AddChild(rpc_leaf[i*9+1]);
      rpc_frame->AddChild(rpc_leaf[i*9+2]);
      rpc_frame->AddChild(rpc_leaf[i*9+3]);
      rpc_frame->AddChild(rpc_leaf[i*9+4]);
      rpc_frame->AddChild(rpc_leaf[i*9+5]);
      rpc_frame->AddChild(rpc_leaf[i*9+6]);
      rpc_frame->AddChild(rpc_leaf[i*9+7]);
      rpc_frame->AddChild(rpc_leaf[i*9+8]);
      rpc->AddChild(rpc_frame);
   }
...
void E88_GeometryV2::SetDesignValues(){
   params.clear();
   geometry_tree->Reset();
   // GTR local translate
   for(int i=0; i<n_gtr_modules; i++){
...
void E88_GeometryV2::SetValuesFromV1(E16ANA_GeometryV1 *geom_v1){

  G4cout << "%%%%% E88_GeometryV2::SetValuesFromV1(E16ANA_GeometryV1 called" << G4endl;
  exit(-1);
   params.clear();
   geometry_tree->Reset();
   ...
   // RPC, sako
   /*   for(int i=0; i<n_rpc_modules; i++){
      double y = geom_v1->RPCy[i]*10.0;
      double z = geom_v1->RPCz[i]*10.0;
      double roty = 30.76*(4-i/3)/180.0*TMath::Pi();
      params[design_str+delimiter+GetTreeStringRPC(i/3,i)].SetXYZ(0.,y,z);
      params[design_str+delimiter+GetTreeStringRPC(i/3)].SetRotation(0.,roty,0.);
      }*/

   SetValues(geometry_tree, params);
}

void E88_GeometryV2::SetValues(Node_t *tree_root, ParamsArray_t &_params){
   tree_root->TranslateAndRotate("", _params);
}

void E88_GeometryV2::PrintParams(std::ostream &ros){
...
void E88_GeometryV2::WriteGeometryFile(const std::string &filename){
  //org   std::ofstream ofs(filename);
  std::ofstream ofs(filename.c_str());
   //std::ofstream ofs(filename.c_str());
   PrintParams(ofs);
}

void E88_GeometryV2::ReadGeometryFile(const std::string &filename, ParamsArray_t &_params){
   _params.clear();
   //org   std::ifstream ifs(filename);
   std::ifstream ifs(filename.c_str());
   while(ifs.good()){
...
std::string E88_GeometryV2::GetTreeStringGTR(int frame){
   return e16_str+delimiter
      +gtr_str+delimiter
      +GetNumString(frame_str, frame);
}

std::string E88_GeometryV2::GetTreeStringGTR(int frame, int layer){
   return GetTreeStringGTR(frame)+delimiter+GetNumString(layer_str, layer);
}

std::string E88_GeometryV2::GetTreeStringGTR(int frame, int layer, int module){
   return GetTreeStringGTR(frame, layer)+delimiter+GetNumString(chamber_str, module);
}

std::string E88_GeometryV2::GetTreeStringHBD(int frame){
   return e16_str+delimiter
      +hbd_str+delimiter
      +GetNumString(frame_str, frame);
}

std::string E88_GeometryV2::GetTreeStringHBD(int frame, int module){
   return GetTreeStringHBD(frame)+delimiter+GetNumString(chamber_str, module);
}

std::string E88_GeometryV2::GetTreeStringLGVD(int frame){
   return e16_str+delimiter
      +lgvd_str+delimiter
      +GetNumString(frame_str, frame);
}

std::string E88_GeometryV2::GetTreeStringLGVD(int frame, int module){
   return GetTreeStringLGVD(frame)+delimiter+GetNumString(chamber_str, module);
}

std::string E88_GeometryV2::GetTreeStringSSD(int frame){
   return e16_str+delimiter
      +ssd_str+delimiter
      +GetNumString(frame_str, frame);
}

std::string E88_GeometryV2::GetTreeStringSSD(int frame, int module){
   return GetTreeStringSSD(frame)+delimiter+GetNumString(chamber_str, module);
}

//sako
std::string E88_GeometryV2::GetTreeStringRPC(int frame){
   return e16_str+delimiter
      +rpc_str+delimiter
      +GetNumString(frame_str, frame);
}

std::string E88_GeometryV2::GetTreeStringRPC(int frame, int module){
   return GetTreeStringRPC(frame)+delimiter+GetNumString(chamber_str, module);
}
