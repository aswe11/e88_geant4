...
//#include "E88_TrackFinding_Nakai_160805.cc"
//#include "E88_TrackFinding_Nakai_160829.cc"
#include "E88_TrackFinding_Nakai_161006.cc"
...
std::vector<int> E88_TrackFinding_Nakai::LinkedModuleIDs(int module){
   std::vector<int> ids;
   if(module/3 == 0){
...
void E16ANA_SingleTrackCandidate_Nakai::SetSingleTrackHit(E16ANA_MultiTrack *single_track, int _track_id){
   ...
   std::vector<int> linked = E88_TrackFinding_Nakai::LinkedModuleIDs(module_id);
   for(int i=1; i<4; i++){
      if(set_flag[i] == 0){
...
E88_TrackFinding_Nakai::E88_TrackFinding_Nakai(
...
void E88_TrackFinding_Nakai::SingleFit(E16ANA_SingleTrackCandidate_Nakai &single, int track_id){
...
void E88_TrackFinding_Nakai::GetSingleTrackCandidates(std::vector<E16ANA_SingleTrackCandidate_Nakai> &_candidates){
...
void E88_TrackFinding_Nakai::GetSingleTrackCandidatesNonPolluted(
      int m_ssd,
   ...
   std::cout << "E88_TrackFinding_Nakai::GetSingleTrackCandidatesNonPolluted()"
             << " : ssd_module = " << m_ssd
             << ", gtr_module = " << m_gtr
             << ", gtr_layer = " << gtr_layer
...
void E88_TrackFinding_Nakai::SingleFitAllCandidates(
...
void E88_TrackFinding_Nakai::ChisqCut(
      std::vector<std::vector<E16ANA_SingleTrackCandidate_Nakai> > &cand,
      double threshold
      ){
   for(int i=0; i<(int)cand.size(); i++){
...
void E88_TrackFinding_Nakai::DoubleFitAllCandidates(
...
void E88_TrackFinding_Nakai::SelectOtherLayerHits(
...
void E88_TrackFinding_Nakai::GetSingleTrackCandidatesY(int m1, int m2, int m3, std::vector<E16ANA_SingleTrackCandidate_Nakai> &cand){
   ...
   std::cout << "E88_TrackFinding_Nakai::GetSingleTrackCandidatesY : # of candidates = " << n_candidates << std::endl;
}

void E88_TrackFinding_Nakai::GetSingleTrackCandidatesV4Y(int m1, int m2, int m3, std::vector<E16ANA_SingleTrackCandidate_Nakai> &cand){
   const int n_div = 10;
   const int n_div1 = 5;
   double size1 = 100.0;
   double size2 = 200.0;
   ...
   std::cout << "E88_TrackFinding_Nakai::GetSingleTrackCandidatesV4Y : # of candidates = " << n_candidates << std::endl;
}


void E88_TrackFinding_Nakai::CheckHBDMatch(std::vector<std::vector<E16ANA_SingleTrackCandidate_Nakai> > &cand, int xy, double match_distance){
   E16ANA_MultiTrack *fitter = new E16ANA_MultiTrack(bfield_map,geom,1);
...
bool E88_TrackFinding_Nakai::IsHBDMatched(E16ANA_SingleTrackCandidate_Nakai &cand, int xy, double match_distance){
   E16ANA_MultiTrack *fitter = new E16ANA_MultiTrack(bfield_map,geom,1);
   std::vector<E16ANA_G4Hit> &hbd_hits = hbd_res->AllAnalyzedHits();
   bool flag = false;
   for(int k=0; k<(int)hbd_hits.size(); k++){ // check all HBD hits
...
TVector3 E88_TrackFinding_Nakai::GetMostPlausibleProjectionHBD(E16ANA_SingleTrackCandidate_Nakai &cand){
   E16ANA_MultiTrack *fitter = new E16ANA_MultiTrack(bfield_map,geom,1);
   std::vector<E16ANA_G4Hit> &hbd_hits = hbd_res->AllAnalyzedHits();
...
void E88_TrackFinding_Nakai::SetG4HitSSD(){
...
void E88_TrackFinding_Nakai::SetG4Hit(int mm){
   {
      int layer = 0;
      int noh = g4event->GTR1().Noh();
      for(int i=0; i<noh; i++){
...
E16ANA_GTRAnalyzedHit* E88_TrackFinding_Nakai::SetG4ClosestTag(std::vector<E16ANA_GTRAnalyzedHit> &hits, int x_or_y, int cog_or_tdc, double g4_near_threshold){
...
void E88_TrackFinding_Nakai::SetG4ClosestTag(std::vector<E16ANA_GTRAnalyzedStripHit> &hits, int cog_or_tdc, double g4_near_threshold){
   double (E16ANA_GTRAnalyzedStripHit::*p_hit)();
   if(cog_or_tdc == 0){p_hit = &E16ANA_GTRAnalyzedStripHit::CogHit;}
   else if(cog_or_tdc == 1){p_hit = &E16ANA_GTRAnalyzedStripHit::Tdc1Hit;}
   else if(cog_or_tdc == 2){p_hit = &E16ANA_GTRAnalyzedStripHit::Tdc2Hit;}
...
//void E88_TrackFinding_Nakai::SetG4ClosestTagSSD(std::vector<E16ANA_G4Hit> &hits, double g4_near_threshold){
//   double res_min = 1000000.0;
//   int i_min = -1;
//   //const double g4_residual_cut = 1.0;
//   for(int i=0; i<(int)hits.size(); i++){
...
void E88_TrackFinding_Nakai::CheckG4ClosestLayer2or3(
      std::vector<std::vector<E16ANA_SingleTrackCandidate_Nakai> > &cand,
...
//void E88_TrackFinding_Nakai::CheckG4Closest(std::vector<std::vector<E16ANA_SingleTrackCandidate_Nakai> > &cand, std::vector<int> &rank, int true_flag){
void E88_TrackFinding_Nakai::CheckG4Closest(
      std::vector<std::vector<E16ANA_SingleTrackCandidate_Nakai> > &cand,
      std::vector<int> &rank, std::vector<TVector3> &hbd_proj, int true_flag){
   rank.resize(cand.size());
   hbd_proj.resize(cand.size());
...
void E88_TrackFinding_Nakai::CheckG4ClosestDouble(
      std::vector<E16ANA_DoubleTrackCandidate_Nakai> &cand,
      std::vector<int> &rank, std::vector<TVector3> &hbd_proj, int true_flag){
   rank.resize(1);
   hbd_proj.resize(2);
...
void E88_TrackFinding_Nakai::PrintNumberOfCandidates(std::vector<std::vector<E16ANA_SingleTrackCandidate_Nakai> > &cand){
   int n = 0;
   for(int i=0; i<(int)cand.size(); i++){
      n += cand[i].size();
   }
...
void E88_TrackFinding_Nakai::ReAnalyzeAndReFit(std::vector<E16ANA_SingleTrackCandidate_Nakai> &cand){
   E16ANA_MultiTrack *fitter = new E16ANA_MultiTrack(bfield_map,geom,1);
   for(int i=0; i<(int)cand.size(); i++){
