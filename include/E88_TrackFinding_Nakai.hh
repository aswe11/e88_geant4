...
#ifndef E88_TrackFinding_Nakai_hh
#define E88_TrackFinding_Nakai_hh
...
public:
   ...
   E16ANA_SingleTrackCandidate_Nakai(E88_GeometryV2 *_geom, int _track_id, double _charge) : 
      mom(0,0,0), vtx(0,0,0), mom_fit(0,0,0), vtx_fit(0,0,0), chisq(0.0)
   {
      geom = _geom;
      track_id = _track_id;
  ...
  //sako
  void AddGTRHit(int layer, E88_G4Hit &hit){
      int layer_id = layer;
      int module_id = hit.ModuleID();
      //      TVector3 lpos(hit.TdcHit(),0.0,0.0);
  ...
  void SetTrackID(int _track_id ) {track_id = _track_id;};
   ...
   double LinearFitY(bool hbd_check, std::vector<E88_G4Hit> &hbd_hits);
   double LinearFitY(){
      std::vector<E88_G4Hit> temp;
      return LinearFitY(false, temp);
   };
   int GetSetFlag(int i){return set_flag[i];};
   //int GetModuleID(int i){return hits[i].module_id;};
   std::vector<E88_G4Hit>& GetAnaHits(int layer){return hits_ana[layer];};
   std::vector<E88_G4Hit>& GetFitHits(int layer){return hits_fit[layer];};
   ...
   TVector3 ProjectionHBD(E16ANA_MultiTrack *fitter, E88_G4Hit &hbd_hit);
   bool HBDMatch(E16ANA_MultiTrack *fitter, E88_G4Hit &hbd_hit, int xy = 3, double match_distance = 50.0);
...
private:
   ...
   std::vector<E88_G4Hit> hits_ana[n_layers];
   std::vector<E88_G4Hit> hits_fit[n_layers];
...
public:
   //static bool SelectionByGTR123(E88_G4Event *g4event);
...
class E88_TrackFinding_Nakai {
public:
   E88_TrackFinding_Nakai(
         E88_G4Event *_g4event,
...
private:
   ...
   E88_G4Hit ssd_g4hit[n_ssd_modules];
   E88_G4Hit gtr_g4hit[n_gtr_modules][3];
   ...
   //void SetG4ClosestTagSSD(std::vector<E88_G4Hit> &hits, double g4_near_threshold = 1.0);
...
#endif // E88_TrackFinding_Nakai_hh
