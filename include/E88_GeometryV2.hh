...
protected:
  //org   std::string detector_type; // "GTR", "HBD", or "SSD"
   std::string detector_type; // "GTR", "HBD", "SSD", or "RPC"
   int module_id;
   int layer_id;
...
class E88_GeometryV2 : public E16ANA_Geometry {
public:
   E88_GeometryV2(); // for testing purposes only
   E88_GeometryV2(const std::string &filename);
   ~E88_GeometryV2();
   ...
   const E16ANA_DetectorGeometry* SSD(int module) const {
      return ssd_geometry[module];
   };
  //sako
   const E16ANA_DetectorGeometry* RPC(int module) const {
     //     G4cout << "E16ANA_DetectorGeometry, module = " << module << G4endl;
     return rpc_geometry[module];
   };
...
private:
  ...
  //sako
   const static std::string rpc_str;
  ...
  //sako
   E16ANA_DetectorGeometry **rpc_geometry;
   ...
   std::string GetTreeStringSSD(int frame, int module);
  //sako
   std::string GetTreeStringRPC(int frame);
   std::string GetTreeStringRPC(int frame, int module);
   std::string GetTreeStringLGVD(int frame);
   std::string GetTreeStringLGVD(int frame ,int module);
};

#endif // E88_GeometryV2_hh
