...
G4IonTable::~G4IonTable()
{
  ...
  if (fIsotopeTableList != 0)
  {
    for (size_t i = 0; i< fIsotopeTableList->size(); ++i)
    {
      ...
      //sako 2018/10/16      if( fIsotopeTable != G4NuclideTable::GetNuclideTable() ) delete fIsotopeTable;
    }
    fIsotopeTableList->clear();
    //sako 2018/10/16    delete fIsotopeTableList;
  }
  fIsotopeTableList =0;
  ...
  //sako 2018/10/16  delete fIonList;
  fIonList =0;
}
...
//G4cout<<"G4IonTable::CreateIon "<<"Z:"<<Z<<" A:"<<A<<" E:"<<E<<" Eex:"<<Eex<<" lvl:"<<lvl<<" name:"<<name<<" code:"<<encoding<<G4endl;
  ...
  /*
   if ((2<=Z)&&(Z<=5)) {
     ...
     G4String name_parent = GetIonName(Z,A,E);
     G4String name_daughter = GetIonName(Z,A-1,E);
     G4cout << "name_parent, name_daughter = " << name_parent << " " << name_daughter << G4endl;
     mode[0] = new G4PhaseSpaceDecayChannel(name_parent,1.0,2,name_daughter,"pi0");

     for (G4int index=0; index <1; index++ ) decayTable->Insert(mode[index]);
...
{
  //  G4cout << "sako in G4IonTable::createIon(Z, A, L, E) = " << Z << " " << A << " " << L << " " << E << G4endl;
  ...
  //                                                         "proton","pi+");
  //  G4VDecayChannel* mode = new G4PhaseSpaceDecayChannel("delta++",1.000, 2,"","pi-";
  ...
  //  G4cout << "sako in createIon, going to GetNucleusMass(Z,A,L), Z, A, L = " << Z << " " << A << " " << L << " " << G4endl;
  ...
  //sako set Lambda lifetime and decay
  //  stable = false;
  //  life = 0.2632*ns;//Lambda(PDG2014)
  //  life = 0.001*ns;//Lambda(PDG2014)
  /*
  bool shortlived = false;//def 
  ion = new G4Ions(   name,            mass,       0.0*MeV,     charge, 
			 J,              +1,             0,          
			 0,               0,             0,             
		 "nucleus",               0,             A,    encoding,
		    stable,            life,    decayTable,       shortlived,
		  "generic",              0,
		      Eex,              lvl         );
  */
  ...
  ion->SetPDGMagneticMoment( -0.613 * mN);
  ...
  decayTable = new G4DecayTable();
    ...
    // create decay channels 
  G4VDecayChannel** mode = new G4VDecayChannel*[1];
    G4String name_parent = GetIonName(Z,A,L,E);
    G4String name_daughter = GetIonName(Z+1,A,E);
    G4cout << "name_parent, name_daughter = " << name_parent << " " << name_daughter << G4endl;
    mode[0] = new G4PhaseSpaceDecayChannel(name_parent,1.0,2,name_daughter,"pi-");
...
{
  //  G4cerr << "sako, G4IonTable apparently wrong CreateIon was called up to now, fixed" << G4endl;
  // G4cout << "Z, A, L, lvl = " << Z << " " << A << " " << L << " " << lvl << G4endl;
  //def  if (L==0) return CreateIon(A,Z,lvl);
  //sako fixed!!!
  ...
  //def  return CreateIon(A,Z,L,0.0);
  //sako fixed!!!
...
{
  //  G4cout << "sako, GetIonTable::GetIon(Z,A,L,lvl) = " << Z << " " << A << " " << L << " " << lvl << G4endl;
...
{
  //  G4cout << "sako in FindIon(Z, A, L, lvl) = " << Z << " " << A << " " << L << " " << lvl << G4endl;
  ...
  //  G4cout << "sako in FindIon, encoding = " << encoding << G4endl;
  G4IonList::iterator i = fIonList->find(encoding);
  for( ;i != fIonList->end() ; i++) {
    ion = i->second;
    if ( ( ion->GetAtomicNumber() != Z) || (ion->GetAtomicMass()!=A) ) break;
...
{
  //  G4cout << "sako, in GetNuclearMass(A, Z, L,lvl), A, Z, L,lvl = " << A << " " << Z << " " << L << " " << lvl << G4endl;
  ...
  if (L == 0) {
    ...
    //    G4cout << "sako, going to G4HyperNucleiProperties::GetNuclearMass(A, Z, L), A, Z, L = " << A << " " << Z << " " << L << G4endl;
...
{
  //  G4cout << "sako in GetIonMass(Z,A,L,lvl), A, Z, L, lvl = " << A << " " << Z << " " << L << " " << lvl << G4endl;
...
//  }
  ...
  if ((pman!=0)&&(ion->GetProcessManager()==0)) {
    if (pmanhyper==0) {
      /*def
      G4String nlambda = "lambda";
      G4ParticleTable *ptable = G4ParticleTable::GetParticleTable();
      G4ParticleDefinition *lambda = ptable->FindParticle(nlambda);
      pmanhyper = lambda->GetProcessManager();
      pmanhyper->DumpInfo();
      //      delete ptable;
      */
      //sako, 2018/10/16
      ...
      G4ParticleDefinition *lambda = G4ParticleTable::GetParticleTable()->FindParticle(nlambda);
      pmanhyper = lambda->GetProcessManager();
      pmanhyper->DumpInfo();
    }

    /*never works!
    if (pman->GetProcessListLength()<=4) {
      //      G4cout << "*** creating pmanhyper ***" << G4endl;

      pmanhyper = new G4ProcessManager(*pman);
      G4cout << "*** adding decay process to pman ***" << G4endl;
      G4Decay *theDecayProcess = new G4Decay();
      //               rest along post
      //transportation  -1 2 3
      //electromagnetic -1 1 2
      //ionIoni         -1  0 1
      //hadronic        -1 -1 0

      //      pmanhyper->AddProcess(theDecayProcess,0,3,4);
      //      pman->RemoveProcess(0);
      pmanhyper->AddProcess(theDecayProcess,0,3,-1);
      //bad      pmanhyper->AddProcess(theDecayProcess,0,3,4);
      //bad      pmanhyper->AddProcess(theDecayProcess,-1,3,4);
      //bad      pmanhyper->AddProcess(theDecayProcess);
      pmanhyper->SetProcessActivation(theDecayProcess,true);
      // set ordering for PostStepDoIt and AtRestDoIt
      //      pmanhyper->SetProcessOrdering(theDecayProcess, idxPostStep);
      //      pmanhyper->SetProcessOrdering(theDecayProcess, idxAtRest);
      //      pmanhyper->DumpInfo();
      pmanhyper->DumpInfo();

      }*/
    //    G4cout << "set pmanhyper" << G4endl;
    ion->SetProcessManager(pmanhyper);
    //    ion->SetProcessManager(pman);
    //    G4cout << "***** genericIon process information (def) *****" << G4endl;
    /*
    if (pman->GetProcessListLength()<=4) {
      G4cout << "*** adding decay process ***" << G4endl;
      G4Decay *theDecayProcess = new G4Decay();
     pman->AddProcess(theDecayProcess);
     // set ordering for PostStepDoIt and AtRestDoIt
     pman->SetProcessOrdering(theDecayProcess, idxPostStep);
     pman->SetProcessOrdering(theDecayProcess, idxAtRest);
     }*/
  } else {
    //    G4cout << "set pmanhyper" << G4endl;
    ion->SetProcessManager(pmanhyper);
  ...
  if ((pman)&&(theDecayProcess->IsApplicable(*ion))) {
     pman->AddProcess(theDecayProcess);
     // set ordering for PostStepDoIt and AtRestDoIt
     pman->SetProcessOrdering(theDecayProcess, idxPostStep);
     pman->SetProcessOrdering(theDecayProcess, idxAtRest);
